...
Choice = 2	#Choose the system type# #1-NVE,2-Andersen,3-NoseHoover,4-MonteCarlo#
		...
		for j in range(0,CycleMultiplication):
			...
			for case in switch(Choice):
				...
				if case (5):
					IntegrateBerendsen()
					break
			Tsum += ma.pow(Velocity, 2.0)
			countV += 1
			if (i<NumberOfInitializationSteps):
				#if (j%PrintFrequency)==0:
				print('%d %f' % (countV, Tsum/countV), file=FileT)
				#countT += 1
			if (i>NumberOfInitializationSteps):
				SampleDiff(2)
				SampleProf(2)
