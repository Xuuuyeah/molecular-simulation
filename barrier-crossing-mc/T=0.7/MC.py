...
Choice = 4	#Choose the system type# #1-NVE,2-Andersen,3-NoseHoover,4-MonteCarlo#
...
def RandomVelocity(temp):
	return ma.pow(temp,2.0)*RandomGaussianNumber()

def RandomGaussianNumber():
	ranA = 0.0
	ranB = 0.0
	ranSq = 0.0

	while( (ranSq >= 1.0) or (ranSq <= 0.0)):
		ranA = 2.0*random.random()-1.0
		ranB = 2.0*random.random()-1.0
		ranSq= ranA*ranA + ranB*ranB
	return ranA*ma.pow(-2.0*ma.log(ranSq)/ranSq,2.0)
...
def IntegrateNVE():
	...
	OldF = ForceBack[1]
	NewPosition = Position + Velocity*Tstep + Tstep*0.5*ma.pow(Tstep, 2.0) #Velocity-Verlet integrator#
	ForceBack = Force(NewPosition,U,F)
	NewVelocity = Velocity + 0.5*(ForceBack[1]+F)*Tstep 
	...
	for i in range(0,nc):
		for j in range(1,6):
			...
			for k in range(0,M-1):
				AA = ma.exp(-(w[j-1]*dt/(8.0*nc))*vlogs[M-k-1])
				vlogs[M-k-2] = vlogs[M-k-2]*ma.pow(AA,2.0)+glogs[M-k-2]*AA*w[j-1]*dt/(4.0*nc)

			AA = ma.exp(-(w[j-1]*dt/(2.0*nc))*vlogs[0])
			scale = scale*AA
			...
			for k in range(0,M-1):
				AA = ma.exp(-(w[j-1]*dt/(8.0*nc))*vlogs[k+1])
...
def IntegrateBerendsen():
	...
	#FutuPosition = Position + Velocity*Tstep + 0.5*OldF*ma.sqrt(Tstep) 
	#PastPosition = Position - Velocity*Tstep + 0.5*OldF*ma.sqrt(Tstep) 
	#Here Velocity means t-0.5*delta(t)#
	#FutuPosition = 2.0*Position - PastPosition + OldF*ma.sqrt(Tstep)
	#Velocity = 0.5*(FutuPosition - PastPosition)/Tstep
...
def IntegrateAndersen():
	...
	#Velocity Verlet#
	#tmpa = 0.0
	#Velocity = NewVelocity + Tstep*0.5*Tstep
	#tmpa = tmpa + ma.sqrt(Velocity)
	#tmpa = tmpa /(Temp*1.0)
	#sigma = ma.sqrt(tmpa)
	#if random.random() <= (Velocity*Tstep):
	#	Velocity = RandomGaussianNumber(sigma)
	Velocity = NewVelocity
	if (random.random()) <= (2*Tstep):
		vx = -1
		while vx<=0:
			vx = np.random.normal((np.sqrt(2*Temp)),(np.sqrt(Temp)))
	...
	for i in range (0,NumberOfSteps):
		for j in range(0,CycleMultiplication):
			...
			if (i<NumberOfInitializationSteps):
				...
				print('%d %f' % (countT,pow((Vsum/countV),2)), file=FileT)
				countT += 1
			if (i>NumberOfInitializationSteps):
				SampleDiff(2)
				SampleProf(2)
				...
				Tt2 += 1.0
				if (j%PrintFrequency)==0:
					#print('%f %f' % (Position,Velocity), file=FilePtr)
					print('%d %f %f %f' % (count,Position,Velocity,pow((Vsum/countV),2)), file=FilePtr)
					count += 1
			if (i==NumberOfInitializationSteps):
